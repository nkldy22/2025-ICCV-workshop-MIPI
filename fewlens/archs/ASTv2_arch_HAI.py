...
class TransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type, isAtt):
        ...
        self.gamma = nn.Parameter(1e-4 * torch.ones(dim), requires_grad=True)

    def forward(self, x):
        if self.isAtt:
            attn_res = self.attn(self.norm1(x))
            x = x + (attn_res * self.gamma.view(1, -1, 1, 1))  

        ffn_res = self.ffn(self.norm2(x))
        x = x + (ffn_res * self.gamma.view(1, -1, 1, 1))
